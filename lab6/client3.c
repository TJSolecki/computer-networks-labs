...
// server over a UDP socket with a rdt 3 protocol that uses ack timeout to
// ensure dropped packets are retransmitted.

// COEN 146L: Lab6, UDP client with packets that have checksum, sequence number,
...
void clientSend(int sockfd, const struct sockaddr *address, socklen_t addrlen,
    ...
    while (1) {
        ...
        if (rv == -1) {
            printf(
                "The return value is -1 meaning an error has occurred during "
                "the call to select\nIs sockfd in readfds? %i\n",
