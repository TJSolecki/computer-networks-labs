...
// Date: 5/8/2024
// Title: Lab6 – Step 2
// Description: This program is a server that listens for a client request to
// upload a file to a a UDP socket using a rdt 3 protocol that uses ack timeout
// to ensure dropped packets are retransmitted.

// COEN 146L: Lab6, UDP server with packets that have checksum, sequence number,
// acknowledgement number, and timer
#include <arpa/inet.h>
#include <fcntl.h>
#include <netdb.h>
...
void serverSend(int sockfd, const struct sockaddr *address, socklen_t addrlen,
    ...
    if (rand() % 5 == 0) { // this give a probability of 20% for a loss of ACK
        ...
        // send packet
        sendto(sockfd, &packet, sizeof(packet), 0, (struct sockaddr *)address,
               addrlen);
...
Packet serverReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen,
    ...
    while (1) {
        ...
        if (packet.header.cksum != e_cksum) {
            ...
            // if the ckechsum and sequence numbers are correct send ack with
