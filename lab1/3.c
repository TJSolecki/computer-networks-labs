...
// Title: Lab1 – Step 3
// Description: This program computes the time taken to copy a file using
// functions from stdio vs. the system functions from fcntl
...
void check_copy_times(char *src_filename, char *dst_filename) {
    clock_t func_start, func_end, sys_start, sys_end;
    ...
    // capture runtime of syscall_copy() using start clock, call the copy, end
    // the clock
    sys_start = clock();
    syscall_copy(src_filename, dst_filename);
    sys_end = clock();
    double syscall_time = ((double)(sys_end - sys_start)) / CLOCKS_PER_SEC;
